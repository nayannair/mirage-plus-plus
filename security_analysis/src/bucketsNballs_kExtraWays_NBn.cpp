...
extra_buckets_struct extra_buckets [NUM_BUCKETS/2];
...
void spill_ball(uns64 index, uns64 ballID){
  ...
  assert ((extra_buckets[index/2].balls_1 + extra_buckets[index/2].balls_0) == EXTRA_BUCKET_CAPACITY+1);
...
uns insert_ball(uns64 ballID){
  ...
  uns64 balls_index1 = bucket[index1]+extra_buckets[index1/2].balls_0 + extra_buckets[index1/2].balls_1;
  uns64 balls_index2 = bucket[index2]+extra_buckets[index2/2].balls_0 + extra_buckets[index2/2].balls_1;
  ...
  retval = bucket[index] + extra_buckets [index/2].balls_0 + extra_buckets [index/2].balls_1;
...
uns64 remove_ball(void){
  ...
  assert((bucket[bucket_index] + extra_buckets[bucket_index/2].balls_0 + extra_buckets[bucket_index/2].balls_1) != 0 );  
  
  bool print_it = false;

  if (bucket_index%2)
  {
    if (extra_buckets[bucket_index/2].balls_1 > 0)
      extra_buckets[bucket_index/2].balls_1--;
    else
      bucket[bucket_index]--;
  }
  else
  {
    if (extra_buckets[bucket_index/2].balls_0 > 0)
      extra_buckets[bucket_index/2].balls_0--;
    else
      bucket[bucket_index]--;
  }
...
void display_histogram(void){
  ...
  for(ii=0; ii< NUM_BUCKETS; ii++){
    s_count[(bucket[ii] + extra_buckets[ii/2].balls_0 + extra_buckets[ii/2].balls_1)]++;
  }
...
void sanity_check(void){
  ...
  for(ii=0; ii< NUM_BUCKETS; ii+=2){
    count = count + extra_buckets[ii/2].balls_0 + extra_buckets[ii/2].balls_1;
  }
...
void init_buckets(void){
  ...
  for(ii=0; ii<NUM_BUCKETS; ii++){
    bucket[ii]=0;
    extra_buckets[ii/2].balls_0 = 0;
    extra_buckets[ii/2].balls_1 = 0;
  }
